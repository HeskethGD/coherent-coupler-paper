...
\begin{abstract}
Complete analytic solutions for the coherent coupler with arbitrary propagation constants and self- and cross-phase modulation coefficients are presented in terms of Weierstrass elliptic $\wp$, $\zeta$, and $\sigma$ functions, providing the full complex envelopes for both modes under arbitrary initial conditions. 
Jensen's coupler emerges as a special case of the general system. 
The solutions take the form of ratios of Weierstrass $\sigma$ functions multiplied by a phase factor whose argument is a logarithm of $\sigma$ ratios, introducing an essential singularity that is shown to be removable by a gauge transformation. 
A projection from the three-mode degenerate four-wave mixing system onto the two-mode coupler is identified, under which the solutions in the degenerate system become single-valued meromorphic Kronecker theta functions. 
...
Despite this long history, closed-form analytic solutions for the full complex envelopes of both modes have not previously been reported for the general system with arbitrary propagation constants and distinct self- and cross-phase modulation coefficients.
...
This progression through the Weierstrass hierarchy — from $\wp$ to $\zeta$ to $\sigma$ — emerges organically from the structure of the complex envelope equations. 
The resulting solutions contain a phase factor whose argument is a logarithm of $\sigma$ ratios, which introduces an essential singularity; we show that a gauge transformation removes this singularity, yielding a cleaner canonical form. 
...
Degenerate FWM is a three-mode parametric process in which two pump photons are converted to a signal-idler pair sharing a common frequency, governed by a system of coupled-mode equations that is known to be integrable \cite{agrawalNonlinearFiberOptics2019, heskethCompleteWeierstrassElliptic2026}. 
...
Section 2 introduces the general coupled-mode equations and the relationship to Jensen's coupler. 
Section 3 derives the Weierstrass elliptic function solutions and the gauge transformation. 
Section 4 establishes the projection from degenerate FWM and the Kronecker theta function form of the solutions. 
Section 5 presents numerical validation. Section 6 concludes with a discussion of implications and possible extensions.
...
Jensesn's system of ordinary differential equations shown in \eqref{eq:jensen} describes propagation in the quasi-continuous-wave limit, i.e., when temporal envelope variations are negligible compared to the spatial evolution along the propagation direction, allowing time derivatives to be neglected.
Physically, the equations govern the evolution of optical waves travelling in two wave guides put in close proximity to each other that exchange energy through coherent coupling of their evanescent fields.
This interaction is modelled by the linear coupling terms parameterised by $\Omega_2$.
Each wave also experiences self-phase modulation (SPM), where its own intensity induces a phase shift on itself, and cross-phase modulation (XPM), where the intensities of the other wave induce additional phase shifts:
...
\begin{itemize}
    ...
    \item $\Omega_1$ are the propagation constants
...
\end{itemize}
We refer the reader to \eqref{eq:jensen} for details on how to express the $\Omega_j$ coefficients in terms of material parameters and overlap integrals. 
Jensen states that the waveguides in \eqref{eq:jensen} were taken to be identical in order to simplify analysis and obtain solutions in standard elliptic integrals, hence the symmetry in the parameters in the two expressions.
In this work however we analytically solve in full a generalised system that does not enforce such symmetry in the $\Omega_j$ parameters.
...
We will use the same notation that we used in \cite{heskethCompleteWeierstrassElliptic2026} for a general FWM system, the only difference being here that there are two modes not four, so mode index $j$ takes values $1,2$, whereas in the FWM case $j=1,2,3,4$. 
...
\end{align}
where in we refer to parameters $a_j, a_{j,k} \in \mathbb{C}$ as propagation constants and phase modulation parameters, respectively, and without loss of generality we take $a_{2,1} = a_{1,2}$.
...
Jensen's system in \eqref{eq:jensen} is recovered from \eqref{eq:uv-system} by the following substitutions:
\begin{align}
a_1 &= - a_2 = - \frac{i {\Omega}_{1}}{{\Omega}_{2}}, \notag \\
a_{1,1} &= a_{2,2} = \frac{i {\Omega}_{3}}{{\Omega}_{2}}, \notag \\
a_{1,2} &= a_{2,1} = - \frac{2 i {\Omega}_{4}}{{\Omega}_{2}}, \notag \\
u_1(z) &= A_1{\left(\frac{z}{{\Omega}_{2}}\right)} e^{- \frac{i \pi}{4}}, \notag \\
u_2(z) &= A_2{\left(\frac{z}{{\Omega}_{2}}\right)} e^{\frac{i \pi}{4}}, \notag \\
v_1(z) &= -A^{*}_1{\left(\frac{z}{{\Omega}_{2}}\right)} e^{\frac{i \pi}{4}}, \notag \\
v_2(z) &= A^{*}_2{\left(\frac{z}{{\Omega}_{2}}\right)} e^{- \frac{i \pi}{4}}, \label{eq:uv-to-jensen}
\end{align}
where $A^*$ denotes the complex conjugate, and where having made the subsitution \eqref{eq:uv-to-jensen} in \eqref{eq:uv-system} one rescales the length variable to a new variable $z'$ such that $z=\Omega_2 z'$.
Therefore, Jensen's system in \eqref{eq:jensen} is a special case of the more general system in \eqref{eq:uv-system}.

In the next section we provide the complete analytic solution for the generalised system in \eqref{eq:uv-system}.
...
The approach and notation follows that of \cite{heskethCompleteWeierstrassElliptic2026} adjusted for two modes instead of four.
...
\begin{equation}
\label{eq:duv_j} 
\frac{d}{d z} u_j v_j = - \prod\limits_{k=1}^{2} u_k + \prod\limits_{k=1}^{2} v_k
\end{equation}
...
The system has two degrees of freedom and two conserved quantities which is a requirement for integrability in the Liouville-Arnold sense \cite{arnoldMathematicalMethodsClassical1989}.
The $\gamma_j$ constants can be determined from \eqref{eq:power-conserved} and initial conditions, 
however, this provides one equation for two unknowns and thus a choice is available for normalisation. 
...
As in the previous section, the approach and notation closely follows that of \cite{heskethCompleteWeierstrassElliptic2026} adjusted for two modes instead of four.
...
\begin{align}
\frac{\frac{\partial}{\partial z} u{\left(z,{\mu}_{j} \right)}}{u{\left(z,{\mu}_{j} \right)}} =& \frac{\left(\zeta{\left(z - z_{0} + z_{1} \right)} - 2 \zeta{\left(z_{1} \right)} - \zeta{\left(z - z_{0} - z_{1} \right)}\right) {\Lambda}_{1,j}}{b_2} \nonumber \\[6pt]
...
\frac{\frac{\partial}{\partial z} v{\left(z,{\mu}_{j} \right)}}{v{\left(z,{\mu}_{j} \right)}} =& - \frac{\left(\zeta{\left(z - z_{0} + z_{1} \right)} - 2 \zeta{\left(z_{1} \right)} - \zeta{\left(z - z_{0} - z_{1} \right)}\right) {\Lambda}_{1,j}}{b_2} \nonumber \\[6pt]
...
\phi_j^{NL}(z)  &= \sum_{k=1}^{N} \left({a}_{j,k} - \frac{1}{N}\sum_{l=1}^{N} {a}_{l,k}\right) \int \hat{u}_k (z) \hat{v}_k (z) \, dz \\
...
\end{align}
It is to be noted that the poles of modes $\hat{u}_j$ are at $z=z_0+z_1$ modulo the lattice and these are not the same poles as their Hamiltonian conjugates $\hat{v}_j$ which are at $z=z_0-z_1$. 
In other work, we have often encountered cases where modes and their conjugates share poles at $z=z_0$ and the modal power is the difference of two Weierstrass $\wp$ functions. 
One may thus ask, what happens if we write these solutions for two modes as the ratio of two such functions, i.e., if we were to write:
\begin{align}
...
\end{align}
then we see that we are recasting the terms on the left of \eqref{eq:two-to-three-ratios-sig} which represent our two mdoes and their conjugates from \eqref{eq:uhat-vhat-gauge-sols}, as three new modes and their conjugates as represented by each term in parentheses on the right hand side of \eqref{eq:two-to-three-ratios-sig}.
One may then ask what would the systme be that  is associated with the newly envisioned three modes and we show in the next section that is familiar to us as degenerate four-wave mixing.

\subsection{Projecting from degenerate FWM to the coherent coupler}\label{subsec:deg-fwm}
In this section we will show how the coherent coupler system in \eqref{eq:uv-system} is essentially equivalent to a degenerate four-wave mixing (FWM) system that one finds in nonlinear fiber optics. 
To be more precise, we will show that there is a projection from the three mode degenerate FWM system to the two mode coherent coupler system.
...
\tilde{u}_j(z) \tilde{v}_j(z) - \tilde{u}_k(z) \tilde{u}_k(z) = \tilde{\gamma}_j - \tilde{\gamma}_k.
\end{equation}

We will now consider a degeneration and rescaling of this FWM system defined by the following substitutions in the Hamiltonian in \eqref{eq:H-deg-tilde} (note the bar notation on the right and the tilde notation on the left):
\begin{align}
...
\begin{align}
  ...
  \frac{\partial}{\partial z} \bar{v}_j(z) &=  -\frac{\partial}{\partial \bar{u}_1} \bar{H}, \notag
\end{align}
\begin{align}
...
\end{align}
We will be showing that there is a projection from \eqref{eq:ubar-vbar-degfwm-system} to \eqref{eq:uv-system}.
...
\end{equation}
then we fix the following relationship betwen the intermodal power constants $\bar{\gamma}_j$ in the degenerate FWM system and the $\gamma_k$ of the coherent coupler system such that:
\begin{alignat}{2}
...
\end{equation}
and the Hamiltonian in the degnerate FWM system takes the value:
\begin{equation}
  \bar{H} = - \frac{{b}_{2} {d}_{5}}{8} - \frac{2}{\chi} + \frac{64 \left({d}_{5} + 16 {\lambda}_{1}\right) {\lambda}_{1}}{\chi^{3} {d}_{5}^{2}}.
\end{equation}
With these paramater relationships the projection is fully defined and one can then verify that by differentiating \eqref{eq:projection-hat-as-bars} with respect to $z$, substituting the differential equations in \eqref{eq:ubar-vbar-degfwm-system} into the right hand side,
then replacing the ratios of modes in bar coordinates for those in hat coordinates using \eqref{eq:projection-hat-as-bars}, and simplifying using the Hamiltonian in \eqref{eq:Hbar-deg-fem},
that one recovers the two mode coherent coupler system in \eqref{eq:uv-hat-system}.
Thus it is established that there exists a projection from degenerate FWM to the coherent coupler.
...
\end{align}
where $\bar{\alpha}_j$ is an integration constant that captures initial relative phase offsets between modes and can be used to equate initial conditions between the degenerate FWM and coherent coupler systems, $s(j)$ is the degeneracy weight such that $s(1)=s(2)=1$ and $s(3)=2$, the new third $\mu_j$ constant is $\mu_3 = z_1 + z_0$, the elliptic functions use invariants $g_2, g_3$, and $\mu_1, \mu_2, z_0, z_1, g_2, g_3$ share the same definitions and values as those given in Section~\ref{sec:mode-power} for the coherent coupler, thereby connecting the two systems. 
The solutions in both \eqref{eq:-ubar-vbar-sols-dfwm} take the form of Kronecker theta functions, which are ratios of two Weierstrass $\sigma$ functions with shifted arguments, multiplied by exponential factors.
...
For plotting purposes we define the following complex valued functions $A_j(z),\ \phi_j(z) \in \mathbb{C}$ that are analagous to modal power (absolute value squared) and phase; they woudl be equivalent to those corresponding real valued functions if $v_j$ was the complex conjugate of $u_j$ rather than the more general Hamiltonian conjugate that we typically consider in our generalised systems:
\begin{align}
A^2_j(z) &= u_j(z)v_j(z), \notag \\
\phi_j(z) &= \frac{\mathrm{log}\left(\frac{u_j(z)}{v_j(z)}\right)}{2}. \label{eq:A-phi-u-v}
\end{align}
...
In comparrison with Figure \ref{fig:case1-plot1}, the modal power $A_j^2$ is unchanged after the gauge transform but the phase $\phi_j$ changes as expected from the definition of the transform.
...
The intermodal power conservation laws in \eqref{eq:power-conserved} are evident from the coordinated evolution of the two modes.
We choose to set $\bar{u}_3(0)=\sigma(z_0 - z_1)/\left(\sigma(z_0)\sigma(z_1)\right)$ and the remaining inital conditions for the other modes in the bar coordinates are then determined using \eqref{eq:projection-hat-as-bars} where the values for $\hat{u}_j(0), \hat{v}_j(0)$ and all other parameters share the same values as thosed used in Figure \ref{fig:case2-plot1}.
...
\caption{\textbf{A} real and \textbf{B} imaginary parts of the complex mode power $A_j^2$, and \textbf{C} and \textbf{D} imaginary parts of the complex phase variable $\phi_j$, using the gauge transformed analytic solutions in \eqref{eq:uhat-vhat-gauge-sols} (lines) compared against numerical integration of the gauge transformed general coherent coupler system in \eqref{eq:uv-hat-system} (symbols).}
...
We have presented complete analytic solutions for the generalised coherent coupler in nonlinear optics without any of the standard simplifying assumptions.
The solutions, expressed in terms of Weierstrass elliptic functions, provide the full complex field envelopes for all modes under arbitrary initial conditions, including cross-phase modulation and phase mismatch effects.
...
Guided by the structure of these solutions, we explore coordinate transformations that revealed a projection from degenerate FWM to the generalised coherent coupler. 
The solutions in the dgenerate FWM system are single-valued meromorphic Kronecker theta functions, connecting this work to a broader class of integrable nonlinear optical systems.
...
2 z_{1} + \sum_{j=1}^{2} {\nu}_{j} &= - 2 \mathrm{N}{\left(0,1 \right)} {\omega}_{1} + 2 \mathrm{N}{\left(1,0 \right)} {\omega}_{3} = 0 \pmod{\text{lattice}}, \label{eq:nuj-sum-zero}  \\
...
\end{align}
We can then use \eqref{eq:uv-wp} to express $\rho$ in terms of $\wp$ so that we can substitute all variables in \eqref{eq:poly-interp} for values in terms of $\wp, \wp'$. 
