...
linearly polarised (LP) modes of a multimode optical fiber \cite{heskethNonlinearEffectsMultimode2014}, two-wave and three-wave mixing in quadratic nonlinear media \cite{heskethGeneralComplexEnvelope2015}, polarisation dynamics in nonlinear fibres \cite{heskethGeneralComplexEnvelope2015}, parity-time symmetric nonlinear couplers \cite{heskethGeneralComplexEnvelope2015}, 
...
\begin{align}
\frac{dE_1}{dZ} &= i \left(|E_1|^2 {\Omega}_{3} + 2 |E_2|^2 {\Omega}_{4}\right) E_1 + i {\Omega}_{1} E_1  + i {\Omega}_{2} E_2, \notag \\
\frac{dE_2}{dZ} &= i \left(2 |E_1|^2 {\Omega}_{4} + |E_2|^2 {\Omega}_{3}\right) E_2 + i {\Omega}_{2} E_1  + i {\Omega}_{1} E_2, \label{eq:jensen}
\end{align}

where:
\begin{itemize}
    \item $E_j$ with $j=1,2$ are the slowly-varying complex field amplitudes
...
u_1(z) &= E_1{\left(Z\right)} e^{- \frac{i \pi}{4}}, \notag \\
u_2(z) &= E^{*}_2{\left(Z\right)} e^{- \frac{i \pi}{4}}, \notag \\
v_1(z) &= -E^{*}_1{\left(Z\right)} e^{\frac{i \pi}{4}}, \notag \\
v_2(z) &= E_2{\left(Z\right)} e^{\frac{i \pi}{4}}, \label{eq:uv-to-jensen}
\end{align}
where $E^*$ denotes the complex conjugate and $z=\Omega_2 Z$ is the rescaled coordinate used in \eqref{eq:uv-system}.
...
In Jensen's case, there is notable simplification in the solutions as the wave guides are made identical.
...
These symmetries further translate into $b_1=0$, $2\Lambda_{1,j}=b_2$, and importantly $r_{1,j} = \frac{1}{2}$, which in \eqref{eq:u-v-quartic-no-wp-sqrt} can be seen to elliminate the logarithmic term in the exponentials, thereby removing the branching behaviour, reducing the solutions to a ratio of sigma functions, and thus, meromorphic functions.
...
In this section we present the second main result of this paper: a transformation from the coherent coupler to a canonical form in which solutions are merormorphic and a projection from degenerate FWM onto that form.
...
The top row of Figure \ref{fig:case3-plot1} shows the real, \textbf{A}, and imaginary, \textbf{B}, parts of the complex mode power $A_j^2$,  
...
Finally, the top row of Figure \ref{fig:case4-plot1} shows the real, \textbf{A}, and imaginary, \textbf{B}, parts of the mode power $A_j^2$,  
...
Here, $E_j, E^{*}_j$ are used in place of $u_j, v_j$ in \eqref{eq:A-phi-u-v} and thus in this case those variables reduce to absolute magnitude squared and the genuine phase, i.e., both become real variables as confirmed by no imaginary parts observed in Figure \ref{fig:case4-plot1} \textbf{B} and \textbf{D}.
The initial mode and parameter values for this plot were $E_1(0)=0.731-0.513j$, $E_2(0)=0.551+0.967i$, $\Omega_1=0.462$, $\Omega_2=-0.00783$, $\Omega_3=-0.541$, and $\Omega_4=0.252$.
...
For the parameter sets shown in Figures~\ref{fig:case1-plot1}--\ref{fig:case4-plot1}, the maximum pointwise absolute discrepancy between the analytic solution vectors and the DOP853 numerical solutions, over the plotted interval and sampled points, was approximately $10^{-11}$, $10^{-13}$, $10^{-13}$, and $10^{-15}$, respectively.
...
\caption{\textbf{A} real and \textbf{B} imaginary parts of the mode power $A_j^2$, and \textbf{C} real and \textbf{D} imaginary parts of the phase variable $\phi_j$, using the analytic solutions in \eqref{eq:u-v-quartic-no-wp-sqrt} and the relations in \eqref{eq:uv-to-jensen} (lines) compared against numerical integration of Jensen's original system in \eqref{eq:jensen} (symbols).}
