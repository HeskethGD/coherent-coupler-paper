...
The approach and notation follows that of \cite{heskethCompleteWeierstrassElliptic2026} adjusted for two mdoes instead of four.
...
As in the previous section, the approach and notation closely follows that of \cite{heskethCompleteWeierstrassElliptic2026} adjusted for two mdoes instead of four.
...
\end{align}
where it is to be noted that in Appendix \ref{app:param-def-appendix} we state $d_4=b_2^2$ and here we take $\sqrt{d_4}=b_2$ and define $z_1$ accordingly.
...
Rather, this is a special instance of the original physical system \eqref{eq:jensen} where parameters satisfy particular constraints.
...
\end{equation}
We emphasize that this is a special case arising from particular parameter values in the four-wave mixing system \eqref{eq:jensen}.
...
In \cite{heskethCompleteWeierstrassElliptic2026}, we showed how quasi-continuous-wave FWM systems can be transformed into the following parameterless canonical Hamiltonian system of four modes $\tilde{u}_j(z), j=1,2,3,4$, and their Hamiltonian conjugates $\tilde{v}_j(z)$ (note the tilde notation):
\begin{align}
...
We then divide the Hamiltonian by $2 \chi^3$, which is equivalent to a change of the length variable such that $z \rightarrow 2 \chi^3 z$ (check this), to obtain the system:
\begin{align}
\bar{H} &= \frac{\tilde{H}}{2 \chi^{3}} = - \frac{\varsigma \chi }{4}\left( \bar{u}_1 \bar{u}_2 \bar{u}^{2}_3 - \bar{v}_1 \bar{v}_2 \bar{v}^{2}_3 \right) + \frac{\chi \bar{u}^{2}_1 \bar{v}^{2}_1}{4} + \frac{\chi \bar{u}^{2}_2 \bar{v}^{2}_2}{4} + \frac{\chi \bar{u}^{2}_3 \bar{v}^{2}_3}{8} - \frac{1}{\chi}\sum_{j=1}^{3}\bar{u}_j \bar{v}_j, 
\end{align}
\begin{alignat}{2}
  \frac{\partial}{\partial t} \bar{u}_1(z) &=  \frac{\partial}{\partial \bar{u}_1} \bar{H} &&= - \frac{\bar{u}_1}{\chi} + \frac{\chi \bar{u}_1^{2} \bar{v}_1}{2} - \frac{\varsigma \chi \bar{v}_2 \bar{v}_3^{2}}{4}, \notag \\
  \frac{\partial}{\partial t} \bar{u}_2(z) &=  \frac{\partial}{\partial \bar{u}_2} \bar{H} &&= - \frac{\bar{u}_2}{\chi} + \frac{\chi \bar{u}_2^{2} \bar{v}_2}{2} - \frac{\varsigma \chi \bar{v}_1 \bar{v}_3^{2}}{4}, \notag \\
  \frac{\partial}{\partial t} \bar{u}_3(z) &=  \frac{\partial}{\partial \bar{u}_3} \bar{H} &&= - \frac{\bar{u}_3}{\chi} + \frac{\chi \bar{u}_3^{2} \bar{v}_3}{4} - \frac{\varsigma \chi \bar{v}_1 \bar{v}_2 \bar{v}_3}{2}, \notag \\
  \frac{\partial}{\partial t} \bar{v}_1(z) &= -\frac{\partial}{\partial \bar{v}_1} \bar{H} &&= \frac{\bar{v}_1}{\chi} - \frac{\chi \bar{v}_1^{2} \bar{u}_1}{2} + \frac{\varsigma \chi \bar{u}_2 \bar{u}_3^{2}}{4}, \notag \\
  \frac{\partial}{\partial t} \bar{v}_2(z) &= -\frac{\partial}{\partial \bar{v}_2} \bar{H} &&= \frac{\bar{v}_2}{\chi} - \frac{\chi \bar{v}_2^{2} \bar{u}_2}{2} + \frac{\varsigma \chi \bar{u}_1 \bar{u}_3^{2}}{4}, \notag \\
  \frac{\partial}{\partial t} \bar{v}_3(z) &= -\frac{\partial}{\partial \bar{v}_3} \bar{H} &&= \frac{\bar{v}_3}{\chi} - \frac{\chi \bar{v}_3^{2} \bar{u}_3}{4} + \frac{\varsigma \chi \bar{u}_1 \bar{u}_2 \bar{v}_3}{2}, \notag \\
\end{alignat}
If one defines modes $\hat{u}_j$, $\hat{v}_j$ with $j=1,2$ (note the hat notation) in terms of the $\bar{u}_k$, $\bar{v}_k$ modes as follows, with $k=1,2,3$:
\begin{align}
  ...
  \hat{v}_2{\left(z\right)} &= \frac{\sqrt{2} \sqrt{{\gamma}_{2} - {\lambda}_{1}} \bar{v}_2{\left(z\right)} e^{z \theta}}{\bar{u}_3{\left(z\right)}}, \notag \\
\end{align}
and sets the values of the conserved quantities in the bar coordinates in terms of parameters from the hat coordinates such that:
\begin{align}
2\bar{u}_1\bar{v}_1 - \bar{u}_3\bar{v}_3 &= \frac{d_5}{4\left(\gamma_1 - \lambda_1\right)}, \notag \\
2\bar{u}_2\bar{v}_2 - \bar{u}_3\bar{v}_3 &= \frac{d_5}{4\left(\gamma_2 - \lambda_1\right)}, \notag \\
\bar{H} &= - \frac{{b}_{2} {d}_{5}}{8} - \frac{2}{\chi} + \frac{64 \left({d}_{5} + 16 {\lambda}_{1}\right) {\lambda}_{1}}{\chi^{3} {d}_{5}^{2}}, \\
\end{align}
then one can confirm after some simple but lengthy algebra that the $\hat{u}_j$, $\hat{v}_j$ modes solve the system in \eqref{eq:uv-hat-system}, thus there exists a projection from degenerate FWM to the coherent coupler.
