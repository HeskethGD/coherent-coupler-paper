...
The solutions are:
\begin{align}
\hat{u}_j{\left(z\right)} &= \frac{{\hat{\alpha}}_{j} \sqrt{W_j}\sigma{\left(z_{1}\right)} \sigma{\left(z - 2 z_{0} + {\mu}_{j}\right)} e^{z\left( {r}_{0,j} -\left(-1\right)^{j} {\beta}\right)} }{ \sigma{\left({\mu}_{j} - z_{0}\right)} \sigma{\left(z - z_{0} - z_{1}\right)}}, \notag \\
\hat{v}_j{\left(z\right)} &= \frac{\sqrt{W_j}\sigma{\left(z_{1}\right)} \sigma{\left(z - {\mu}_{j}\right)} e^{-z\left( {r}_{0,j} -\left(-1\right)^{j} {\beta} \right)}}{ {\hat{\alpha}}_{j} \sigma{\left({\mu}_{j} - z_{0}\right)} \sigma{\left(z - z_{0} + z_{1}\right)}},
\end{align}
where:
\begin{equation}
\beta = \frac{\left(- {r}_{1,1} + {r}_{1,2}\right) \left(- 2 \zeta{\left(z_{1}\right)} + {b}_{2} {\gamma}_{1} + {b}_{2} {\lambda}_{1}\right)}{2} - 2 {a}_{1,2} {\gamma}_{1} - {a}_{2} - \frac{{b}_{1}}{2}
\end{equation}
...
\bar{u}_j{\left(z\right)} \bar{v}_j{\left(z\right)} &= \frac{s{\left(j \right)}}{2}\left(\wp{\left({\mu}_{j} - z_{0} \right)} - \wp{\left(z - z_{0}\right)}\right)
\end{align}
